...
public class ProgressItem extends AbstractFlexibleItem<ProgressItem.ProgressViewHolder> {
    ...
    public void bindViewHolder(FlexibleAdapter adapter, ProgressViewHolder holder,
        ...
        switch (this.status) {
            case NO_MORE_LOAD:
                holder.progressMessage.setText("no_more_load_retry");
                // Reset to default status for next binding
                setStatus(StatusEnum.MORE_TO_LOAD);
                break;
            case DISABLE_ENDLESS:
                holder.progressMessage.setText("endless_disabled");
                break;
            case ON_CANCEL:
                holder.progressMessage.setText("endless_cancel");
                // Reset to default status for next binding
                setStatus(StatusEnum.MORE_TO_LOAD);
                break;
            case ON_ERROR:
                holder.progressMessage.setText("endless_error");
                // Reset to default status for next binding
                setStatus(StatusEnum.MORE_TO_LOAD);
                break;
            default:
